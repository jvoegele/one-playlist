-- Vendored from usebasejump/supabase-test-helpers v0.0.6 (MIT licence):
-- https://github.com/usebasejump/supabase-test-helpers
--
-- This is the manual-install path from their docs: paste the extension's
-- body into your test suite's first-alphabetical file, rather than
-- installing it as a real `CREATE EXTENSION` via dbdev (which would need
-- network access to database.dev on every machine that runs the tests).
--
-- Naming this `00000-...` makes it sort before every other file in
-- supabase/tests/, and `supabase test db` runs every .sql file under that
-- directory as its own script regardless of name (see docs/learning-log.md,
-- 2026-09-14) — so this one runs first, creates the `tests` schema and its
-- functions, and every later test file can call them. `CREATE SCHEMA IF NOT
-- EXISTS` / `CREATE OR REPLACE FUNCTION` are the load-bearing idempotent
-- pieces here: unlike our reverted \ir attempt, this file only defines
-- *functions*, never inserts fixture rows itself, so re-running it (or
-- having it execute standalone, un-transacted) is harmless.
--
-- Two changes from upstream:
--   1. Dropped the `\echo ... \quit` guard at the top of the original —
--      that only matters when a file is loaded via `CREATE EXTENSION`
--      (which silently skips backslash lines); loaded directly by psql, as
--      this one is, it would just quit before defining anything.
--   2. `extensions.uuid_generate_v4()` → `gen_random_uuid()`, since this
--      project doesn't otherwise depend on `uuid-ossp` and every migration
--      here already uses the built-in `gen_random_uuid()`.
--   3. Added a one-assertion plan/finish at the bottom, upstream has neither
--      — a standalone script with no TAP output at all still gets recorded
--      as a failed file by `supabase test db` (verified locally), even
...
select plan(1);
select pass('tests schema and helper functions created');
select * from finish();
